...
const suites=['src/tests/threat-remediation-postgres.integration.test.ts','src/tests/threat-governance-postgres.integration.test.ts','src/tests/threat-model-postgres.integration.test.ts'];
for(const suite of suites)if(!existsSync(suite))throw new Error(`Required integration suite missing: ${suite}`);
...
try {
  ...
  let ready=false;for(let n=0;n<60;n++){const r=spawnSync('docker',['exec',name,'pg_isready','-h','127.0.0.1','-U','postgres'],{windowsHide:true});if(r.status===0){ready=true;break;}await new Promise(r=>setTimeout(r,500));}if(!ready)throw new Error('Disposable PostgreSQL did not become ready');
