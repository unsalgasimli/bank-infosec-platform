...
import { mapDepartment, getDepartmentColor, getDepartmentIcon, generateDeptCode, slugifyDept } from './src/server/services/ldap-directory.data.js';

function processDatabaseFile(filePath) {
  ...
  console.log(`\nProcessing ${filePath}: ${db.users?.length || 0} users, ${db.departments?.length || 0} departments`);

  const syncedDepartmentIds = new Set();
  ...
  for (const user of db.users || []) {
    ...
    let deptRecord = (db.departments || []).find((d) => d.id === user.departmentId);
    if (!deptRecord) {
      deptRecord = {
  ...
  // 2. Resolve department managers and admin user IDs
  for (const dept of db.departments || []) {
    const deptMembers = (db.users || []).filter((u) => u.departmentId === dept.id && u.isActive !== false);
    ...
    if (!dept.managerId || !db.users.some((u) => u.id === dept.managerId)) {
