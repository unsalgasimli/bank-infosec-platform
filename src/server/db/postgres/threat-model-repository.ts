...
export class ThreatModelRepository {
  static async list(scope: { userId: string; departmentId?: string; elevated: boolean; limit?: number; offset?: number; classifications: string[] }): Promise<Row[]> {
    const result = await pgClient.query<Row>(
      `SELECT tm.*, r.revision_number, r.status AS revision_status
        ...
        WHERE ($1::boolean OR tm.business_owner_id = $2 OR tm.technical_owner_id = $2 OR tm.security_owner_id = $2)
          AND tm.data_classification=ANY($5::varchar[])
        ORDER BY tm.updated_at DESC,tm.id LIMIT $3 OFFSET $4`,
      [scope.elevated, scope.userId, scope.limit || 50, scope.offset || 0, scope.classifications]
    );
  ...
  static component(row: Row): Row { return { contentVersion: Number(row.content_version || 1), securityZone: row.security_zone, id: row.id, revisionId: row.revision_id, name: row.name, type: row.type, description: row.description || undefined, technology: row.technology || undefined, assetId: row.asset_id || undefined, ownerId: row.owner_id || undefined, criticality: row.criticality || undefined }; }
