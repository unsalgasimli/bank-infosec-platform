-- Resolve renewal lineage through the normalized append-only membership table.
CREATE OR REPLACE FUNCTION guard_threat_exception_renewal() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE prior varchar(64); current_threat threats; rid varchar(64); mid varchar(64); review threat_exception_escalation_reviews; current_lineage varchar(64); model_status varchar(32);
BEGIN
  SELECT t.* INTO current_threat FROM threats t WHERE t.id=NEW.threat_id;
  SELECT lineage_id INTO current_lineage FROM threat_lineage_members WHERE threat_id=NEW.threat_id;
  ...
  SELECT e.id INTO prior FROM threat_model_exceptions e JOIN threat_lineage_members p ON p.threat_id=e.threat_id WHERE p.lineage_id=current_lineage AND e.approved_at IS NOT NULL AND e.id<>NEW.id ORDER BY e.approved_at DESC,e.id DESC LIMIT 1;
