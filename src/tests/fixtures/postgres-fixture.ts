...
function installOrchestrationFixture(data: DatabaseSchema, owner: any): void {
  ...
  basic('wf-software-feature-delivery', 'Software Feature Delivery', 'SOFTWARE_DEVELOPMENT', [
    start, node('feature-qa', 'feature-qa', 'TASK', { assignment: { strategy: 'FIXED_PERSON', assigneeId: owner.id } }), node('feature-security', 'feature-security', 'TASK', { assignment: { strategy: 'FIXED_PERSON', assigneeId: owner.id } }), node('feature-join', 'feature-join', 'PARALLEL_JOIN', { join: { strategy: 'ALL' } }), node('feature-approval', 'feature-approval', 'APPROVAL', { approval: { approverSource: 'ROLE', role: 'INFOSEC_MANAGER', approvalMode: 'ANY_ONE', timeoutMinutes: 60, reminderMinutes: 15, escalationSource: 'CAB_BOARD', preventSelfApproval: true } }), node('feature-deploy', 'feature-deploy', 'SYSTEM_ACTION', { action: { actionKey: 'DEPLOY' } }), finish,
  ], [edge('e1', 'start', 'feature-qa'), edge('e2', 'start', 'feature-security'), edge('e3', 'feature-qa', 'feature-join'), edge('e4', 'feature-security', 'feature-join'), edge('e5', 'feature-join', 'feature-approval'), edge('e6', 'feature-approval', 'feature-deploy', { outcome: 'APPROVED' }), edge('e7', 'feature-deploy', 'finish')]);
  ...
  ], [edge('e1', 'start', 'change-gate'), edge('e2', 'change-gate', 'change-approval', { outcome: 'FALSE' }), edge('e3', 'change-gate', 'change-risk', { outcome: 'TRUE' }), edge('e4', 'change-approval', 'change-risk', { outcome: 'APPROVED' }), edge('e5', 'change-risk', 'change-deploy'), edge('e6', 'change-deploy', 'change-rollback', { outcome: 'FAILED' }), edge('e7', 'change-rollback', 'change-incident'), edge('e8', 'change-incident', 'change-finalize'), edge('e9', 'change-finalize', 'change-failed'), edge('e10', 'change-deploy', 'finish', { outcome: 'SUCCEEDED' })]);
  ...
  if (onboardingVersion) {
    onboardingVersion.stages = [
      { id: 'onboard-preboarding', key: 'preboarding', title: 'Preboarding', order: 1, trigger: 'IMMEDIATE', nodeIds: ['start', 'onboard-identity', 'onboard-laptop', 'onboard-employment', 'onboard-contractor-approval', 'onboard-remote', 'onboard-location', 'onboard-remote-equipment', 'onboard-facilities'] },
