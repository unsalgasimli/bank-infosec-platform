        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
          {queues.map((q) => (
            <div
              ...
              className="p-4 bg-bank-900 border border-slate-800 hover:border-slate-700 rounded-lg space-y-2 cursor-pointer transition-colors group"
            >
              <div className="flex items-center justify-between">
                <span className="font-semibold text-white text-xs group-hover:text-blue-400 transition-colors">
                  {q.name}
                </span>
                <span className="px-2 py-0.5 rounded bg-slate-800 text-blue-300 font-mono text-xs font-semibold">
                  {q.count || 0}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800 text-[10px] text-slate-500 font-mono">
                ...
                <span className="text-blue-400 group-hover:underline flex items-center gap-1 font-sans font-medium">
      ...
      <div className="bg-bank-900 border border-slate-800 rounded-lg p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
          {workload.map((item, idx) => (
            <div key={idx} className="p-3 bg-bank-950 border border-slate-800 rounded-md space-y-2">
              <div className="flex items-center justify-between">
              ...
              <div className="w-full h-1.5 bg-bank-900 rounded-full overflow-hidden border border-slate-800">
                <div
                  className={`h-full ${item.criticalCount > 0 ? 'bg-red-500' : 'bg-blue-600'}`}
                  style={{ width: `${Math.min(100, item.count * 15)}%` }}
                />
              </div>
              <div className="flex items-center justify-between text-[10px] text-slate-500">
                <span>Critical: <strong className="text-red-400">{item.criticalCount}</strong></span>
