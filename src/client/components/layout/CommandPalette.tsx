...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  const quickViews = [
    ...
    { label: 'Vulnerability Management & Scanners', view: 'vulnerabilities', icon: Layers },
    { label: 'Enterprise Risk Register (5x5 Matrix)', view: 'risk-register', icon: FileText },
    { label: 'Security Exceptions & Expirations', view: 'security-exceptions', icon: CheckCircle2 },
    { label: 'Knowledge Base & Playbooks', view: 'knowledge-base', icon: FileText },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 p-4">
      <div className="fixed inset-0 bg-bank-950/80 backdrop-blur-sm" onClick={onClose} />
      <div className="relative w-full max-w-2xl bg-bank-900 border border-slate-700/80 rounded-xl shadow-2xl overflow-hidden z-10">
        <div className="flex items-center px-4 py-3 border-b border-slate-800 bg-bank-850">
          ...
          <input
            ...
            placeholder="Type a command, ticket key (e.g. APPSEC-2026-0001), or switch user..."
        ...
        <div className="max-h-96 overflow-y-auto p-2 space-y-4 text-xs">
          ...
          {quickViews.length > 0 && (
            <div>
              <div className="px-3 py-1 font-bold uppercase tracking-wider text-slate-400 text-[10px]">
                Navigation & Views
              </div>
                ...
                {filteredTickets.map((t) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5 truncate">
                      <span className="font-mono font-bold text-navy-300 text-xs">{t.key}</span>
                      <span className="truncate text-slate-200">{t.title}</span>
                    </div>
                    <span className="text-[10px] uppercase font-bold text-slate-400 ml-2">{t.technicalSeverity}</span>
                  </button>
                ))}
              </div>
            </div>
