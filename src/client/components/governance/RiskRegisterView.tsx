    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-[#F4F5F7] custom-scrollbar">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#DFE1E6] pb-2 text-xs">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-[10px] font-bold uppercase tracking-wider text-[#7A869A] mr-1">Treatment:</span>
          {[
      ...
      {/* Risk Table */}
      <div className="bg-[#FFFFFF] border border-[#DFE1E6] rounded-md p-5 space-y-3 shadow-sm">
        ...
        {filteredRisks.length === 0 ? (
          <div className="py-16 text-center text-[#5E6C84] text-xs italic bg-[#FFFFFF] rounded border border-[#DFE1E6]">
          ...
          <div className="divide-y divide-[#DFE1E6] text-xs">
            {filteredRisks.map((risk) => (
              <div
                ...
                className="py-3 flex flex-col md:flex-row md:items-center justify-between gap-3 hover:bg-[#EBECF0] px-2.5 rounded transition-colors group cursor-pointer"
              >
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[#5E6C84] text-[10px]">→ Residual:</span>
                    <Badge type="SEVERITY" value={risk.residualRating} size="sm" />
                    <span className="px-2 py-0.5 rounded bg-[#FFFFFF] text-[#0052CC] border border-[#DFE1E6] text-[10px] font-mono font-semibold">
                      {risk.treatmentStrategy}
                    </span>
                  </div>
                  <h4 className="text-xs font-semibold text-[#172B4D] group-hover:text-[#0052CC]">{risk.title}</h4>
                  <p className="text-[11px] text-[#5E6C84] leading-relaxed">{risk.description}</p>
                  <div className="text-[11px] text-[#006644] pt-0.5 flex items-center gap-1.5 font-medium">
                    <CheckCircle2 className="w-3.5 h-3.5 shrink-0" />
                    <span><strong className="text-[#172B4D]">Treatment:</strong> {risk.treatmentPlan} (Target: {risk.treatmentDeadline})</span>
                  </div>
                </div>

                <div className="text-right shrink-0">
                  <div className="text-xs font-mono font-bold text-[#172B4D]">
                    Inherent: <span className="text-[#DE350B] font-bold">{risk.inherentScore}</span> / 25
                  </div>
                  <div className="text-[11px] text-[#5E6C84] mt-0.5">Owner: <strong className="text-[#172B4D]">{risk.ownerName}</strong></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-[#FFFFFF] border border-[#DFE1E6] rounded-md max-w-lg w-full p-5 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#DFE1E6] pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-[#172B4D]">New Enterprise Risk Assessment</h3>
              </div>
            ...
            <form onSubmit={handleCreateRisk} className="space-y-3 text-xs">
              <div>
                <label className="block text-[#5E6C84] mb-1">Risk Title / Threat Statement:</label>
                <input
                  type="text"
                  value={title}
              ...
              <div>
                <label className="block text-[#5E6C84] mb-1">Detailed Context & Risk Scenario:</label>
                <textarea
                  value={description}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#5E6C84] mb-1">Likelihood (1 to 5):</label>
                  <select
                    value={likelihood}
                ...
                <div>
                  <label className="block text-[#5E6C84] mb-1">Impact (1 to 5):</label>
                  <select
                    value={impact}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#5E6C84] mb-1">Treatment Strategy:</label>
                  <select
                ...
                <div>
                  <label className="block text-[#5E6C84] mb-1">Target Deadline:</label>
                  <input
                    type="date"
                    value={deadline}
              ...
              <div>
                <label className="block text-[#5E6C84] mb-1">Actionable Remediation / Treatment Plan:</label>
                <input
                  type="text"
                  value={treatmentPlan}
