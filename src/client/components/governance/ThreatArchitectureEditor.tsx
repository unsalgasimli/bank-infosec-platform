...
export function ThreatArchitectureEditor({detail,mutable,fetchWithAuth,onRefresh}:Props){
  ...
  const options=(values:readonly string[])=>values.map(id=>({id}));
  const triState=(name:string,label:string)=><label>{label}<select className={inputClass} value={draft[name]===null||draft[name]===undefined?'':String(draft[name])} onChange={event=>setDraft({...draft,[name]:event.target.value===''?null:event.target.value==='true'})}><option value="">Unknown / not assessed</option><option value="true">Yes</option><option value="false">No</option></select></label>;
  ...
  return <section className="space-y-4">
    <ThreatArchitectureDiagram detail={detail} onSelect={(nextKind,entityId)=>{const entity=(nextKind==='component'?detail.components:detail.dataFlows).find(item=>item.id===entityId);if(!entity)return;setKind(nextKind);setDraft({...entity,dataTypesText:(entity.dataTypes||[]).join(', ')});setReason('');setError('');setNotice('');setConfirmRemove(false);}}/>
    ...
    <form onSubmit={event=>{event.preventDefault();void save('UPDATE');}}><fieldset disabled={!mutable||busy||stale} className="grid md:grid-cols-2 gap-3">
      ...
      {kind==='component'&&<>
        {select('type','Component type',options(architectureComponentTypes))}{field('securityZone','Security zone',true)}{field('technology','Technology')}{field('assetId','Canonical asset ID')}{field('ownerId','Canonical owner ID')}{select('criticality','Criticality',options(['LOW','MEDIUM','HIGH','CRITICAL']),true)}
        {select('exposure','Exposure',options(architectureExposures))}{field('authenticationMethod','Authentication method')}{select('privileges','Runtime privileges',options(architecturePrivileges))}{select('hosting','Hosting',options(architectureHosting))}{select('environment','Environment',options(architectureEnvironments))}
        <p>Before moving a component across zones, link the affected flows to a boundary. The server rejects unguarded crossings. Processed/stored information assets are linked in the Data tab.</p>
      </>}
      ...
      {kind==='flow'&&<>
        {select('sourceComponentId','Source',detail.components)}{select('destinationComponentId','Destination',detail.components)}{select('trustBoundaryId','Trust boundary',detail.trustBoundaries,true)}{field('protocol','Protocol')}
        <label>Port<input className={inputClass} type="number" min={1} max={65535} value={draft.port??''} onChange={event=>setDraft({...draft,port:event.target.value})}/></label>
        {field('authenticationMethod','Authentication method')}{field('authorizationContext','Authorization context',false,true)}{triState('encryptionInTransit','Encryption in transit')}{field('encryptionMechanism','Encryption mechanism')}{field('encryptionVersion','Encryption version')}{select('integrityProtection','Integrity protection',options(architectureIntegrity))}
        {triState('internetExposure','Internet exposure')}{triState('thirdPartyInvolvement','Third-party involvement')}{select('logging','Logging coverage',options(architectureLogging))}{field('purpose','Purpose',false,true)}
        {select('dataClassification','Data classification',options(['PUBLIC','INTERNAL','RESTRICTED','CONFIDENTIAL_SECURITY_ONLY','HIGHLY_RESTRICTED_HR_LEGAL']))}{field('dataTypesText','Data types (comma-separated)')}{select('direction','Direction',options(['ONE_WAY','BIDIRECTIONAL']))}{field('notes','Flow security assumptions',false,true)}
        <p>Crossing is derived from persisted component zones. Information assets are linked in the Data tab. Classification can raise, but never automatically lower, the model's confidentiality label. Unknown context is not evidence of protection. If transport is unencrypted, remove encryption mechanism/version claims before saving.</p>
      </>}
