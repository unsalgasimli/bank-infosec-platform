    ...
    {draft&&<>
      ...
      <form onSubmit={save}><fieldset disabled={!mutable||busy||stale} className="grid gap-3">
        ...
        <details><summary>Attacker, architecture links and ownership</summary><div className="grid md:grid-cols-2 gap-3 mt-3">
          <label>Methodology<select className={fieldClass} value={draft.methodology||'STRIDE'} onChange={event=>setDraft({...draft,methodology:event.target.value})}>{['STRIDE','ABUSE_CASE','ATTACK_TREE','OTHER'].map(value=><option key={value}>{value}</option>)}</select></label>
          <p>Source: {draft.source||'LEGACY_UNSPECIFIED'} · Analysis v{draft.analysisVersion||1}</p>
          {textField('assumptions','Security assumptions',false,true)}
          {['confidentialityImpact','integrityImpact','availabilityImpact'].map(name=><label key={name}>{name}<input className={fieldClass} type="number" min={1} max={5} value={draft[name]??''} onChange={event=>setDraft({...draft,[name]:event.target.value===''?null:Number(event.target.value)})}/></label>)}
          <fieldset><legend>Affected security properties</legend>{['CONFIDENTIALITY','INTEGRITY','AVAILABILITY','AUTHENTICITY','ACCOUNTABILITY','NON_REPUDIATION'].map(value=><label key={value} className="block"><input type="checkbox" checked={(draft.securityProperties||[]).includes(value)} onChange={event=>setDraft({...draft,securityProperties:event.target.checked?[...(draft.securityProperties||[]),value]:(draft.securityProperties||[]).filter((item:string)=>item!==value)})}/> {value}</label>)}</fieldset>
