...
export const DiscoveryAdminView: React.FC<{ mode: AdminMode; onNavigateToRuns?: (connectorId: string) => void }> = ({ mode, onNavigateToRuns }) => {
  ...
  const openCreate = () => { setEditing(null); setNotice(''); setError(''); setForm({ ...emptyForm, connectionId: '' }); setFormOpen(true); };
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) throw new Error(data.error || t('Could not save connector.'));
      ...
      setFormOpen(false); setNotice(editing ? t('vCenter connector updated.') : t('vCenter connector added.')); await load();
  ...
  return <div className="flex-1 overflow-y-auto w-full p-6 space-y-5 bg-semantic-subtle custom-scrollbar">
    ...
    {mode === 'sources' && connectors.some((connector) => connector.latestRun) && <div className="wrike-card flex flex-wrap gap-x-6 gap-y-2 p-3 text-xs text-semantic-muted">{connectors.filter((connector) => connector.latestRun).map((connector) => <span key={`${connector.id}:latest-run`}><strong className="text-semantic-primary">{connector.name}:</strong> {connector.latestRun.state} â€¢ {connector.latestRun.discoveredCount ?? 0} {t('discovered')} / {connector.latestRun.failedCount ?? 0} {t('failed')}</span>)}</div>}
