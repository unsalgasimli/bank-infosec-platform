...
import { Activity, AlertCircle, CheckCircle2, Edit3, Loader2, Play, PlugZap, Plus, RefreshCw, ServerCog, ShieldAlert, TestTube2, ToggleLeft, ToggleRight, Trash2, X } from 'lucide-react';
...
import { SmbPrinterConnectorModal } from './SmbPrinterConnectorModal.js';
import { LibreNmsConnectorModal } from './LibreNmsConnectorModal.js';

type AdminMode = 'sources' | 'runs';
...
const connectorLabel = (connectorType?: string) => connectorType === 'ACTIVE_DIRECTORY' ? 'Active Directory' : connectorType === 'CORTEX' ? 'Palo Alto Cortex' : connectorType === 'VCENTER' ? 'VMware vCenter' : connectorType === 'SMB_PRINTER' ? 'SMB printer shares (read-only)' : connectorType === 'LIBRENMS' ? 'LibreNMS network inventory' : connectorType || 'Unknown';
const connectorEndpoint = (connector: any) => {
  ...
  if (connector.connectorType === 'SMB_PRINTER') return connector.nonSecretConfiguration?.host ? `\\\\${connector.nonSecretConfiguration.host}` : '—';
  if (connector.connectorType === 'LIBRENMS') return connector.nonSecretConfiguration?.endpointUrl || '—';
  return '—';
};
const connectorVersion = (connector: any) => connector.vcenter?.detectedVersion || connector.detectedVersion || (connector.connectorType === 'ACTIVE_DIRECTORY' ? 'LDAPS' : connector.connectorType === 'CORTEX' ? 'HTTPS API' : connector.connectorType === 'SMB_PRINTER' ? 'SMB • Read-only' : connector.connectorType === 'LIBRENMS' ? 'HTTPS API • v0' : undefined);
...
async function requestDiscovery(fetchWithAuth: (url: string, options?: RequestInit) => Promise<Response>, url: string, options?: RequestInit): Promise<{ response: Response; data: any }> {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      const response = await fetchWithAuth(url, { ...options, cache: 'no-store', signal: controller.signal });
      return { response, data: await discoveryJson(response) };
    } catch (error) {
      if (error instanceof DOMException && error.name === 'AbortError') throw new Error('The CMDB request did not finish within 45 seconds. Verify vCenter reachability, TLS and API service health, then try again.');
      lastError = error;
...
export const DiscoveryAdminView: React.FC<{ mode: AdminMode; onNavigateToRuns?: (connectorId: string) => void }> = ({ mode, onNavigateToRuns }) => {
  ...
  const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [notice, setNotice] = useState(''); const [selectedConnector, setSelectedConnector] = useState(''); const [cortexInventoryScope, setCortexInventoryScope] = useState<'ENDPOINTS' | 'ASSETS'>('ENDPOINTS'); const [page, setPage] = useState(1); const [total, setTotal] = useState(0); const pageSize = 25;
  const [formOpen, setFormOpen] = useState(false); const [adFormOpen, setAdFormOpen] = useState(false); const [cortexFormOpen, setCortexFormOpen] = useState(false); const [smbPrinterFormOpen, setSmbPrinterFormOpen] = useState(false); const [libreNmsFormOpen, setLibreNmsFormOpen] = useState(false); const [editingManagedConnector, setEditingManagedConnector] = useState<any | null>(null); const [editing, setEditing] = useState<any | null>(null); const [form, setForm] = useState<FormState>(emptyForm); const [saving, setSaving] = useState(false); const [formError, setFormError] = useState(''); const [actionId, setActionId] = useState(''); const [health, setHealth] = useState<any | null>(null); const [testResult, setTestResult] = useState<any | null>(null); const [coverage, setCoverage] = useState<any | null>(null); const [dailyBatch, setDailyBatch] = useState<any | null>(null);
  ...
  const load = async (background = false) => {
    ...
    try {
      if (mode === 'runs' && connectors.length === 0) { const { response, data } = await requestDiscovery(fetchWithAuth, '/api/cmdb/discovery/connectors'); if (response.ok && data.success) setConnectors((data.connectors || []).filter((item: any) => ['VCENTER', 'ACTIVE_DIRECTORY', 'CORTEX', 'SMB_PRINTER', 'LIBRENMS'].includes(item.connectorType))); }
      const endpoint = mode === 'sources' ? '/api/cmdb/discovery/connectors' : selectedConnector ? `/api/cmdb/discovery/connectors/${encodeURIComponent(selectedConnector)}/runs?limit=${pageSize}` : null;
      ...
      if (mode === 'sources') {
        const discovery = (data.connectors || []).filter((item: any) => ['VCENTER', 'ACTIVE_DIRECTORY', 'CORTEX', 'SMB_PRINTER', 'LIBRENMS'].includes(item.connectorType));
        ...
        try {
          const batchResponse = await requestDiscovery(fetchWithAuth, '/api/cmdb/discovery/daily-sync');
          if (batchResponse.response.ok && batchResponse.data.success) setDailyBatch(batchResponse.data.batch || null);
        } catch { setDailyBatch(null); }
      } else {
  ...
  const hasActiveRuns = mode === 'sources' ? connectors.some(connectorHasActiveRun) || dailyBatch?.state === 'RUNNING' : items.some((item) => item.state === 'QUEUED' || item.state === 'RUNNING');
  useEffect(() => {
  ...
  const title = mode === 'sources' ? t('Discovery Sources') : t('Inventory Sync Runs');
  ...
  const runAction = async (connector: any, action: 'enable' | 'disable' | 'test-connection' | 'health') => {
    ...
    try {
      const { response, data } = await requestDiscovery(fetchWithAuth, `/api/cmdb/discovery/connectors/${encodeURIComponent(connector.id)}/${action}`, {
        method: action === 'health' ? 'GET' : 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      if (!response.ok || !data.success) {
        const failure = data.details || {
          status: 'FAILED',
          code: data.code,
          message: data.error || t('Connector action failed.'),
        };
        if (action === 'health') setHealth({ connector, metrics: failure, checkedAt: new Date().toISOString(), liveProbe: false });
        if (action === 'test-connection') setTestResult(failure);
        throw new Error(data.error || t('Connector action failed.'));
      }
      if (action === 'health') {
        setHealth(data);
      } else {
        if (action === 'test-connection') {
          // vCenter/AD return snapshot.testResult while Cortex returns the
          // connector-neutral testResult envelope. Accept both contracts and
          // fall back to the full safe response so a successful test is never
          // silently discarded.
          setTestResult(data.snapshot?.testResult ?? data.testResult ?? data);
        }
        setNotice(action === 'test-connection' ? t('Connection test completed successfully.') : t('Connector state updated.'));
        await load();
      }
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : t('Connector action failed.'));
    } finally {
      setActionId('');
    }
  };
  ...
  const runDailySync = async () => {
    setActionId('daily-sync'); setError(''); setNotice('');
    try {
      const { response, data } = await requestDiscovery(fetchWithAuth, '/api/cmdb/discovery/daily-sync', { method: 'POST', headers: { 'Content-Type': 'application/json' } });
      if (!response.ok || !data.success) throw new Error(discoveryFailureMessage(response, data, t('Daily CMDB sync could not be started.')));
      setDailyBatch(data.batch || null);
      setNotice(t('Daily CMDB sync started. Sources will run one at a time and reconcile through the canonical CMDB.'));
      await load(true);
    } catch (cause) { setError(cause instanceof Error ? cause.message : t('Daily CMDB sync could not be started.')); }
    finally { setActionId(''); }
  };
  ...
  return <div className="flex-1 overflow-y-auto w-full p-6 space-y-5 bg-semantic-subtle custom-scrollbar" onInvalidCapture={(event) => { if (!formOpen) return; const field = event.target as HTMLInputElement; const label = field.labels?.[0]?.textContent?.trim() || field.name || t('Required field'); setFormError(`${label}: ${field.validationMessage || t('This field is required.')}`); }}>
    <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-semantic-info"><ServerCog className="w-5 h-5" /><span className="text-xs font-bold uppercase tracking-wider">{t('Assets & CMDB')}</span></div><h1 className="text-xl font-bold text-semantic-primary mt-1">{title}</h1><p className="text-xs text-semantic-muted mt-1">{mode === 'sources' ? t('Connector-scoped infrastructure and identity evidence. Secrets stay server-side; discovery never bypasses correlation.') : t('Auditable, connector-scoped inventory execution history.')}</p>{!loading && <p className="mt-1 text-xs text-semantic-muted">{hasActiveRuns ? t('Live updates every 2.5 seconds while a sync is running.') : t('Live updates are enabled.')}</p>}</div><div className="flex flex-wrap justify-end gap-2"><button onClick={() => void load()} className="jira-btn-subtle"><RefreshCw className="w-4 h-4" />{t('Refresh')}</button>{mode === 'sources' && <><button onClick={() => void runDailySync()} disabled={Boolean(actionId) || dailyBatch?.state === 'RUNNING'} className="jira-btn-primary"><Play className="w-4 h-4" />{t('Daily sync & correlate all')}</button><button onClick={() => void bootstrapActiveDirectory()} disabled={Boolean(actionId)} className="jira-btn-primary"><Plus className="w-4 h-4" />{t('Use existing Active Directory')}</button><button onClick={() => setAdFormOpen(true)} className="jira-btn-subtle"><Plus className="w-4 h-4" />{t('Add Active Directory')}</button><button onClick={() => setSmbPrinterFormOpen(true)} className="jira-btn-subtle"><Plus className="w-4 h-4" />{t('Add SMB printers')}</button><button onClick={() => { setEditingManagedConnector(null); setLibreNmsFormOpen(true); }} className="jira-btn-subtle"><Plus className="w-4 h-4" />{t('Add LibreNMS')}</button><button onClick={() => setCortexFormOpen(true)} className="jira-btn-subtle"><Plus className="w-4 h-4" />{t('Add Cortex')}</button><button onClick={openCreate} className="jira-btn-subtle"><Plus className="w-4 h-4" />{t('Add vCenter')}</button></>}</div></div>
    ...
    {mode === 'sources' && dailyBatch && <section className={`wrike-card p-4 ${dailyBatch.state === 'RUNNING' ? 'border-semantic-info/40' : dailyBatch.state === 'SUCCEEDED' ? 'border-semantic-success/40' : 'border-amber-500/40'}`} aria-live="polite"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-xs font-bold uppercase tracking-wide text-semantic-muted">{t('Daily sync & correlation')}</div><div className="mt-1 font-semibold text-semantic-primary">{dailyBatch.state} · {dailyBatch.completedCount}/{dailyBatch.sourceCount} {t('sources completed')}</div><p className="mt-1 text-xs text-semantic-muted">{dailyBatch.state === 'RUNNING' ? t('The next source is released only after the current source reaches a terminal result.') : dailyBatch.completedAt ? `${t('Completed')}: ${isoLabel(dailyBatch.completedAt)}` : ''}</p></div><span className={dailyBatch.failedCount ? 'text-sm font-bold text-amber-600' : 'text-sm font-bold text-semantic-success'}>{dailyBatch.failedCount ? `${dailyBatch.failedCount} ${t('attention')}` : t('Canonical correlation')}</span></div><div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4">{(dailyBatch.items || []).map((item: any) => <div key={`${dailyBatch.id}:${item.sequence}`} className="rounded-lg bg-semantic-subtle p-2 text-xs"><div className="flex justify-between gap-2"><span className="font-medium text-semantic-primary">{item.sequence}. {item.connectorName}</span><span>{item.state}</span></div><div className="mt-1 text-semantic-muted">{item.runId ? `${item.discoveredCount || 0} ${t('discovered')} · ${item.failedCount || 0} ${t('failed')}` : t('Waiting for prior source')}</div>{item.checkpoint && <div className="mt-1 truncate text-micro text-semantic-muted" title={item.checkpoint}>{item.checkpoint}</div>}{item.failureMessage && <div className="mt-1 text-semantic-danger">{item.failureMessage}</div>}</div>)}</div></section>}
    {!loading && <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">{mode === 'sources' ? <><div className="wrike-card p-4"><div className="text-xs font-medium uppercase tracking-wide text-semantic-muted">{t('Canonical assets')}</div><div className="mt-1 text-2xl font-bold text-semantic-primary">{coverage?.totalCanonicalAssets ?? 0}</div></div><div className="wrike-card p-4"><div className="text-xs font-medium uppercase tracking-wide text-semantic-muted">{t('Cortex endpoint coverage')}</div><div className="mt-1 text-2xl font-bold text-semantic-success">{coverage?.cortexCoveragePercent ?? 0}%</div><div className="text-xs text-semantic-muted">{coverage?.cortexManaged ?? 0}/{coverage?.endpointAssets ?? 0} {t('endpoints')} · {coverage?.cortexStale ?? 0} {t('stale')}</div></div><div className="wrike-card p-4"><div className="text-xs font-medium uppercase tracking-wide text-semantic-muted">{t('Reconciliation required')}</div><div className="mt-1 text-2xl font-bold text-amber-600">{(coverage?.reconciliationRequired ?? 0) + (coverage?.identityConflicts ?? 0)}</div><div className="text-xs text-semantic-muted">{coverage?.adWithoutCortex ?? 0} AD {t('endpoints without Cortex')}</div></div></> : mode === 'runs' ? <><div className="wrike-card p-4"><div className="text-xs font-medium uppercase tracking-wide text-semantic-muted">{t('Visible runs')}</div><div className="mt-1 text-2xl font-bold text-semantic-primary">{total}</div></div><div className="wrike-card p-4"><div className="text-xs font-medium uppercase tracking-wide text-semantic-muted">{t('Latest discovered')}</div><div className="mt-1 text-2xl font-bold text-semantic-success">{items[0]?.discoveredCount ?? 0}</div></div><div className="wrike-card p-4"><div className="text-xs font-medium uppercase tracking-wide text-semantic-muted">{t('Latest failed')}</div><div className="mt-1 text-2xl font-bold text-semantic-danger">{items[0]?.failedCount ?? 0}</div></div></> : <><div className="wrike-card p-4"><div className="text-xs font-medium uppercase tracking-wide text-semantic-muted">{t('Evidence records')}</div><div className="mt-1 text-2xl font-bold text-semantic-primary">{total}</div></div><div className="wrike-card p-4"><div className="text-xs font-medium uppercase tracking-wide text-semantic-muted">{t('Awaiting correlation')}</div><div className="mt-1 text-2xl font-bold text-amber-600">{unmatchedEvidence}</div></div><div className="wrike-card p-4"><div className="text-xs font-medium uppercase tracking-wide text-semantic-muted">{t('Correlated')}</div><div className="mt-1 text-2xl font-bold text-semantic-success">{Math.max(0, items.length - unmatchedEvidence)}</div></div></>}</div>}
