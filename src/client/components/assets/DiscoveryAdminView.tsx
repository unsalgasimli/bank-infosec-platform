...
const connectorLabel = (connectorType?: string) => connectorType === 'ACTIVE_DIRECTORY' ? 'Active Directory' : connectorType === 'CORTEX' ? 'Palo Alto Cortex' : connectorType === 'VCENTER' ? 'VMware vCenter' : connectorType === 'SMB_PRINTER' ? 'SMB printer shares (read-only)' : connectorType || 'Unknown';
...
const connectorVersion = (connector: any) => connector.vcenter?.detectedVersion || connector.detectedVersion || (connector.connectorType === 'ACTIVE_DIRECTORY' ? 'LDAPS' : connector.connectorType === 'CORTEX' ? 'HTTPS API' : connector.connectorType === 'SMB_PRINTER' ? 'SMB • Read-only' : undefined);
...
export const DiscoveryAdminView: React.FC<{ mode: AdminMode; onNavigateToRuns?: (connectorId: string) => void }> = ({ mode, onNavigateToRuns }) => {
  ...
  const load = async (background = false) => {
    ...
    try {
      if (mode === 'runs' && connectors.length === 0) { const { response, data } = await requestDiscovery(fetchWithAuth, '/api/cmdb/discovery/connectors'); if (response.ok && data.success) setConnectors((data.connectors || []).filter((item: any) => ['VCENTER', 'ACTIVE_DIRECTORY', 'CORTEX', 'SMB_PRINTER'].includes(item.connectorType))); }
      ...
      if (mode === 'sources') {
        const discovery = (data.connectors || []).filter((item: any) => ['VCENTER', 'ACTIVE_DIRECTORY', 'CORTEX', 'SMB_PRINTER'].includes(item.connectorType));
