  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-semantic-subtle custom-scrollbar select-none">
      <div className="wrike-card p-6 bg-gradient-to-r from-semantic-panel via-semantic-subtle to-semantic-success-surface/30 border border-semantic-border shadow-sm flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-semantic-primary">Business Service Posture</h1>
          <p className="text-xs text-semantic-muted mt-0.5">Derived from authorized CMDB applications and active tickets. Availability is shown only when a monitoring integration provides it.</p>
        </div>
      </div>
          ...
          {applications.map((application) => {
            ...
            return (
              <div key={application.id} className="wrike-card p-5 space-y-4 hover:border-semantic-success transition-all shadow-xs">
                ...
                <p className="text-xs text-semantic-muted leading-relaxed">{application.description || 'No CMDB description has been supplied.'}</p>
                <div className="grid grid-cols-2 gap-3 p-3 rounded-lg bg-semantic-subtle border border-semantic-border text-xs">
                  <div><span className="text-semantic-muted">Open tickets</span><div className="font-mono font-bold text-semantic-primary mt-0.5">{activeTickets.length}</div></div>
                  <div><span className="text-semantic-muted">Urgent / SLA risk</span><div className={`font-mono font-bold mt-0.5 ${requiresAttention ? 'text-semantic-danger' : 'text-semantic-success'}`}>{urgentTickets.length}</div></div>
                </div>
                <div className="text-xs text-semantic-muted flex items-center justify-between"><span>Monitoring telemetry: <strong className="text-semantic-secondary">not connected</strong></span>{urgentTickets[0] && <button onClick={() => onSelectTicket?.(urgentTickets[0])} className="text-semantic-info font-bold hover:underline">Open ticket →</button>}</div>
              </div>
            );
          })}
        </div>
