            ...
            <div className="mx-auto grid max-w-6xl gap-7 min-[900px]:grid-cols-[minmax(0,1.8fr)_minmax(270px,0.9fr)] min-[900px]:gap-8">
              <section aria-labelledby="content-heading" className="min-w-0">
                ...
                <div className="mt-5 space-y-4">
                  <label className="block"><span className="mb-1.5 block text-[13px] font-semibold text-semantic-strong">{t('Title')} <span className="text-red-600">*</span></span><input ref={titleRef} value={form.title} onChange={(event) => update('title', event.target.value)} onBlur={() => { if (form.title.trim() && form.title.trim().length < 3) setError(t('Title must be at least 3 characters.')); }} placeholder={example} aria-invalid={Boolean(error && form.title.trim().length < 3)} className="h-11 w-full rounded-xl border border-semantic-border-strong px-3 text-sm font-medium outline-none transition placeholder:font-normal placeholder:text-semantic-placeholder focus:border-semantic-brand focus:ring-4 focus:ring-semantic-brand/10" /></label>
                  <label className="block"><span className="mb-1.5 block text-[13px] font-semibold text-semantic-strong">{t('Description')} <span className="text-red-600">*</span></span><span className="block overflow-hidden rounded-xl border border-semantic-border-strong transition focus-within:border-semantic-brand focus-within:ring-4 focus-within:ring-semantic-brand/10"><span className="flex flex-wrap items-center gap-1 border-b border-semantic-border bg-slate-50/70 px-2 py-1.5"><span className="mr-1 text-[11px] font-semibold text-semantic-muted">{t('Quick structure:')}</span>{[t('What is the problem?'), t('Impact'), t('Expected outcome')].map((prompt) => <button key={prompt} type="button" onClick={() => insertPrompt(prompt)} className="rounded-lg px-2 py-1 text-[11px] font-semibold text-semantic-muted transition hover:bg-white hover:text-semantic-brand focus:outline-none focus:ring-2 focus:ring-semantic-brand/20">{prompt}</button>)}</span><textarea ref={descriptionRef} rows={7} value={form.description} onChange={(event) => update('description', event.target.value)} onBlur={() => { if (!form.description.trim()) setError(t('Description is mandatory.')); }} placeholder={t('Enter context, affected system and expected outcome...')} aria-invalid={Boolean(error && !form.description.trim())} className="block min-h-[168px] w-full resize-y border-0 px-3 py-3 text-sm leading-relaxed outline-none placeholder:text-semantic-placeholder" /></span></label>
                ...
                {!intake?.directory.ready && <div className="mt-5 flex gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" /><span><strong>{t('Live AD synchronization required.')}</strong> {intake?.directory.message}</span></div>}
              </section>
