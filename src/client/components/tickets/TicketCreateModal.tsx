...
const priorityMeta: Record<BusinessPriority, { short: string; label: string; response: string; resolution: string; tone: string }> = {
  P1_URGENT: { short: 'P1', label: 'Kritik', response: '15 dəq.', resolution: '2 iş saatı', tone: 'border-red-500/40 bg-red-500/15 text-red-400 font-bold' },
  P2_HIGH: { short: 'P2', label: 'Yüksək', response: '1 iş saatı', resolution: '1 iş günü', tone: 'border-amber-500/40 bg-amber-500/15 text-amber-400 font-bold' },
  P3_MEDIUM: { short: 'P3', label: 'Orta', response: '4 iş saatı', resolution: '2 iş günü', tone: 'border-emerald-500/40 bg-emerald-500/15 text-emerald-400 font-bold' },
  P4_LOW: { short: 'P4', label: 'Aşağı', response: '8 iş saatı', resolution: '5 iş günü', tone: 'border-slate-500/40 bg-slate-500/15 text-slate-300 font-bold' },
};
...
const SearchableDropdown = ({ id, value, onChange, options, placeholder, disabled, searchPlaceholder, ariaLabelledBy }: {
  ...
  return (
    <div ref={fieldRef} className="relative w-full">
      <div onClick={(event) => { if (!disabled && event.target !== inputRef.current) { inputRef.current?.focus(); openAndSearch(); } }} className={`flex h-11 w-full items-center rounded-xl border bg-semantic-panel shadow-xs transition ${open ? 'border-semantic-brand ring-4 ring-semantic-brand/10' : 'border-semantic-border-strong hover:border-semantic-brand/70 hover:bg-semantic-subtle'} ${disabled ? 'border-semantic-border bg-semantic-subtle opacity-60' : ''}`}>
        <input
          ref={inputRef}
          id={id}
          type="text"
...
export const TicketCreateModal: React.FC<TicketCreateModalProps> = ({ isOpen, onClose, onCreated }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-dsDialog flex items-center justify-center bg-semantic-modal-tint/60 p-3 backdrop-blur-sm sm:p-5">
      <section role="dialog" aria-modal="true" aria-labelledby="new-work-title" className="flex h-[min(780px,calc(100vh-24px))] w-[min(1120px,100%)] flex-col overflow-hidden rounded-[26px] border border-semantic-border-strong bg-semantic-panel shadow-2xl sm:h-[min(780px,calc(100vh-40px))]">
        <header className="flex shrink-0 items-center justify-between border-b border-semantic-border px-5 py-4 sm:px-8 sm:py-5">
          <div className="flex min-w-0 items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-semantic-success-surface text-semantic-success"><Plus className="h-5 w-5" /></span><div className="min-w-0"><div className="flex min-w-0 items-center gap-2"><h2 id="new-work-title" className="shrink-0 text-base font-bold text-semantic-primary">{t('New Ticket')}</h2><span className="max-w-[230px] truncate rounded-full bg-semantic-neutral-surface px-2.5 py-1 text-[11px] font-bold text-semantic-strong">{selectedCategoryLabel}</span></div><p className="truncate text-xs text-semantic-muted">{t('Create request - routing and SLA are calculated automatically')}</p></div></div>
          <button type="button" onClick={requestClose} aria-label={t('Close')} className="rounded-xl p-2 text-semantic-muted transition hover:bg-semantic-neutral-surface hover:text-semantic-primary focus:outline-none focus:ring-4 focus:ring-semantic-brand/10"><X className="h-5 w-5" /></button>
        </header>

        <main className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-8 sm:py-6">
          {loading ? <div className="flex h-full items-center justify-center gap-3 text-sm font-medium text-semantic-muted"><Loader2 className="h-5 w-5 animate-spin text-semantic-brand" /> {t('Loading details...')}</div> : (
            <div className="mx-auto grid max-w-6xl gap-7 min-[900px]:grid-cols-[minmax(0,1.8fr)_minmax(270px,0.9fr)] min-[900px]:gap-8">
              <section aria-labelledby="content-heading" className="min-w-0">
                <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-semantic-success">{t('01 · Content')}</p><h3 id="content-heading" className="mt-2 text-xl font-bold tracking-tight text-semantic-primary">{t('Describe your request')}</h3>
                <div className="mt-5 space-y-4">
                  <label className="block"><span className="mb-1.5 block text-[13px] font-semibold text-semantic-strong">{t('Title')} <span className="text-red-500">*</span></span><input ref={titleRef} value={form.title} onChange={(event) => update('title', event.target.value)} onBlur={() => { if (form.title.trim() && form.title.trim().length < 3) setError(t('Title must be at least 3 characters.')); }} placeholder={example} aria-invalid={Boolean(error && form.title.trim().length < 3)} className="h-11 w-full rounded-xl border border-semantic-border-strong bg-semantic-panel px-3 text-sm font-medium text-semantic-primary outline-none transition placeholder:font-normal placeholder:text-semantic-placeholder focus:border-semantic-brand focus:ring-4 focus:ring-semantic-brand/10" /></label>
                  <label className="block"><span className="mb-1.5 block text-[13px] font-semibold text-semantic-strong">{t('Description')} <span className="text-red-500">*</span></span><span className="block overflow-hidden rounded-xl border border-semantic-border-strong bg-semantic-panel transition focus-within:border-semantic-brand focus-within:ring-4 focus-within:ring-semantic-brand/10"><span className="flex flex-wrap items-center gap-1 border-b border-semantic-border bg-semantic-subtle px-2 py-1.5"><span className="mr-1 text-[11px] font-semibold text-semantic-muted">{t('Quick structure:')}</span>{[t('What is the problem?'), t('Impact'), t('Expected outcome')].map((prompt) => <button key={prompt} type="button" onClick={() => insertPrompt(prompt)} className="rounded-lg px-2 py-1 text-[11px] font-semibold text-semantic-muted transition hover:bg-semantic-panel hover:text-semantic-brand focus:outline-none focus:ring-2 focus:ring-semantic-brand/20">{prompt}</button>)}</span><textarea ref={descriptionRef} rows={7} value={form.description} onChange={(event) => update('description', event.target.value)} onBlur={() => { if (!form.description.trim()) setError(t('Description is mandatory.')); }} placeholder={t('Enter context, affected system and expected outcome...')} aria-invalid={Boolean(error && !form.description.trim())} className="block min-h-[168px] w-full resize-y border-0 bg-transparent px-3 py-3 text-sm leading-relaxed text-semantic-primary outline-none placeholder:text-semantic-placeholder" /></span></label>
                  <SelectField label={t('Affected CMDB Element')} value={affectedCiId} onChange={setAffectedCiId} searchable searchPlaceholder={t('Search CI number, name or host...')} placeholder={t('Select CI (Optional)')} hint={t('Selected element is linked to the ticket; business impact is calculated from CMDB dependencies.')} options={ciOptions} />
                  <div onDragEnter={(event) => { event.preventDefault(); setDragActive(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragActive(false)} onDrop={(event) => { event.preventDefault(); setDragActive(false); addFiles(event.dataTransfer.files); }} className={`rounded-xl border border-dashed px-4 py-3 transition ${dragActive ? 'border-semantic-brand bg-semantic-brand/5' : 'border-semantic-border-strong bg-semantic-subtle/50 hover:border-semantic-brand/50 hover:bg-semantic-subtle'}`}><input ref={fileInputRef} type="file" multiple className="sr-only" onChange={(event) => { if (event.target.files) addFiles(event.target.files); event.currentTarget.value = ''; }} /><button type="button" onClick={() => fileInputRef.current?.click()} className="flex w-full items-center gap-3 text-left focus:outline-none focus:ring-4 focus:ring-semantic-brand/10"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-semantic-border bg-semantic-panel text-semantic-muted shadow-sm"><UploadCloud className="h-4 w-4" /></span><span className="min-w-0"><span className="block text-sm font-semibold text-semantic-strong">{t('Attach file')}</span><span className="block text-xs text-semantic-muted">{t('Drag here or browse · up to 25 MB per file')}</span></span><Paperclip className="ml-auto h-4 w-4 shrink-0 text-semantic-placeholder" /></button>{pendingFiles.length > 0 && <div className="mt-3 flex flex-wrap gap-2 border-t border-semantic-border pt-3">{pendingFiles.map((file, index) => <span key={`${file.name}-${index}`} className="inline-flex max-w-full items-center gap-1.5 rounded-lg border border-semantic-border bg-semantic-subtle px-2.5 py-1.5 text-xs font-medium text-semantic-strong shadow-sm"><FileText className="h-3.5 w-3.5 text-semantic-muted" /><span className="max-w-[220px] truncate">{file.name}</span><button type="button" onClick={() => setPendingFiles((current) => current.filter((_, fileIndex) => fileIndex !== index))} aria-label={`${file.name} faylını sil`} className="ml-1 rounded p-0.5 text-semantic-muted hover:bg-red-500/10 hover:text-red-400"><X className="h-3 w-3" /></button></span>)}</div>}</div>
                </div>
                {!intake?.directory.ready && <div className="mt-5 flex gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-400"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" /><span><strong>{t('Live AD synchronization required.')}</strong> {intake?.directory.message}</span></div>}
              </section>
       ...
       <section aria-labelledby="routing-heading"><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-semantic-success">{t('02 · Routing')}</p><h3 id="routing-heading" className="sr-only">{t('Routing')}</h3><div className="mt-4 flex items-center gap-2 rounded-xl border border-semantic-border bg-semantic-subtle px-3 py-2.5"><UserRound className="h-4 w-4 text-semantic-success" /><span className="min-w-0 truncate text-sm font-bold text-semantic-primary">{intake?.requester.fullName || t('Current User')}</span><span className="ml-auto text-[11px] text-semantic-muted">{t('Requester')}</span></div><div className="mt-4 space-y-4"><SelectField label={t('Category')} value={form.category} onChange={changeCategory} recommended searchable searchPlaceholder={t('Search category...')} placeholder={t('Select category')} options={[{ value: '', label: t('Select category') }, ...(intake?.categories || []).map((category) => ({ value: category.code, label: category.label, sublabel: category.kind === 'BASIC_TICKET' ? `${category.catalogGroup || 'IT'} · Help Desk task` : category.description }))]} /><SelectField label={t('Target Department / Section')} value={form.targetId} onChange={changeTarget} disabled={!intake?.directory.ready} recommended={Boolean(categoryRecommendation && (!form.targetId || targetWasAutoSelected))} searchable searchPlaceholder={t('Search section, department or team...')} placeholder={t('Select unit')} hint={categoryRecommendation && (!form.targetId || targetWasAutoSelected) ? `${categoryRecommendation.name} ${t('suggested based on category.')}` : undefined} options={[{ value: '', label: t('Select unit') }, ...targetOptions.map((unit) => ({ value: unit.id, label: unit.name, sublabel: unit.sublabel }))]} /><SelectField label={t('Assignee')} value={form.assigneeId} onChange={(value) => update('assigneeId', value)} disabled={assigneeSelectionLocked} searchable searchPlaceholder={t('Search employee by name or role...')} placeholder={assigneePlaceholder} hint={assigneeHint} options={[{ value: '', label: assigneePlaceholder }, ...(intake?.assignees || []).map((person) => ({ value: person.id, label: person.fullName, sublabel: [person.sectionName, person.title].filter(Boolean).join(' / ') }))]} /></div>{selectedTarget && <p className="mt-3 flex items-start gap-1.5 text-xs text-semantic-muted"><ArrowRight className="mt-0.5 h-3.5 w-3.5 shrink-0 text-semantic-success" /> {form.assigneeId ? t('Will be directly assigned to chosen employee.') : `${t('Will be routed to queue:')} “${selectedTarget.name}”.`}</p>}</section>

       <section aria-labelledby="priority-heading" className="mt-7 border-t border-semantic-border pt-6"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-semantic-success">{t('03 · Priority & SLA')}</p><h3 id="priority-heading" className="mt-2 text-lg font-bold tracking-tight text-semantic-primary">{t('How urgent is this?')}</h3></div><Sparkles className="h-4 w-4 text-semantic-success" /></div><div className="mt-4 grid grid-cols-4 gap-1.5" role="radiogroup" aria-label="Prioritet">{(Object.keys(priorityMeta) as BusinessPriority[]).map((value) => { const meta = priorityMeta[value]; const selected = value === selectedPriority; return <button key={value} type="button" role="radio" aria-checked={selected} onClick={() => changePriority(value)} className={`rounded-lg border px-1 py-2 text-center transition focus:outline-none focus:ring-4 focus:ring-semantic-brand/10 ${selected ? meta.tone : 'border-semantic-border bg-semantic-panel text-semantic-muted hover:border-semantic-border-strong hover:bg-semantic-subtle'}`}><span className="block text-xs font-extrabold">{meta.short}</span><span className="mt-0.5 block text-[10px] font-semibold">{t(meta.label)}</span>{selected && <Check className="mx-auto mt-1 h-3 w-3" />}</button>; })}</div><div key={`priority-summary-${selectedPriority}`} className="mt-3 rounded-xl bg-semantic-success-surface/70 px-3 py-2.5"><div className="flex items-center justify-between gap-3"><span className="text-xs font-semibold text-semantic-success">{priority.short} {t(priority.label)} · {t('target')}</span><CheckCircle2 className="h-4 w-4 text-semantic-success" /></div><p className="mt-1 text-xs text-semantic-strong">{t('Response:')} {priority.response} <span className="mx-1 text-semantic-muted">·</span> {t('Resolution:')} {priority.resolution}</p></div><div className="mt-4"><SelectField label={t('SLA Policy')} value={form.slaPolicyId} onChange={(value) => update('slaPolicyId', value)} recommended searchable searchPlaceholder={t('Search SLA policy...')} placeholder={t('System Default')} hint={selectedSla?.description || t('Selection verified by server.')} options={[{ value: '', label: t('System Default') }, ...(intake?.slaPolicies || []).map((policy) => ({ value: policy.id, label: policy.name, sublabel: policy.description }))]} /></div><div className="mt-4 flex items-start gap-2 text-xs text-semantic-muted"><UsersRound className="mt-0.5 h-3.5 w-3.5 shrink-0 text-semantic-success" /><span>{selectedTarget ? `${selectedTarget.name} → ${form.assigneeId ? t('chosen assignee') : t('queue')} · ${priority.short} · ${selectedSla?.name || t('System Default')}` : t('Routing details will appear here once department is selected.')}</span></div></section>
              </aside>
            </div>
          )}
          {error && <div role="alert" className="mx-auto mt-5 flex max-w-6xl items-start gap-2 rounded-xl border border-red-500/40 bg-red-500/10 px-4 py-3 text-sm text-red-400"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" /><span>{error}</span></div>}
        </main>

        <footer className="flex shrink-0 flex-col gap-3 border-t border-semantic-border bg-semantic-panel px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-8"><span className="flex items-center gap-2 text-xs font-semibold text-semantic-success"><CheckCircle2 className="h-4 w-4 shrink-0" /> {t('Priority and SLA will be automatically calculated.')}</span><div className="flex justify-end gap-2"><button type="button" onClick={requestClose} className="rounded-xl border border-semantic-border-strong px-4 py-2.5 text-sm font-bold text-semantic-strong transition hover:bg-semantic-subtle focus:outline-none focus:ring-4 focus:ring-semantic-brand/10">{createdTicketKey ? t('Close') : t('Cancel')}</button><button type="button" disabled={loading || submitting || Boolean(createdTicketKey)} onClick={() => void submit()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-semantic-brand px-5 py-2.5 text-sm font-bold text-slate-950 transition hover:brightness-105 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 shadow-[0_0_16px_-2px_rgba(0,245,118,0.4)]">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}{submitting ? t('Creating...') : createdTicketKey ? t('Created') : t('Create Ticket')}</button></div></footer>
      </section>
    </div>,
    document.body,
  );
