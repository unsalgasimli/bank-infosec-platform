  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-50/60 border border-slate-200 rounded-xl p-5 shadow-xs">
        ...
        <div className="text-xs text-slate-800 leading-relaxed whitespace-pre-line font-normal bg-semantic-panel p-4 rounded-lg border border-slate-200">
          {ticket.description || t('No description provided.')}
        </div>
      </div>
      ...
      {(ticket.resolutionCode || ticket.resolutionSummary) && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-5 shadow-xs">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold uppercase tracking-wider text-emerald-800">{t('Resolution Outcome')}</h3>
          </div>
          <div className="mt-2 text-xs text-slate-800">
            <strong className="text-emerald-900 font-bold">{ticket.resolutionCode?.replaceAll('_', ' ') || t('Resolved')}</strong>
      ...
      {cmdb.length > 0 && (
        <div className="rounded-xl border border-blue-200 bg-blue-50/60 p-5 shadow-xs">
          <div className="flex items-center gap-2"><Server className="h-4 w-4 text-blue-700" /><h3 className="text-xs font-bold uppercase tracking-wider text-blue-900">{t('Affected Configuration Items & Business Impact')}</h3></div>
          <div className="mt-3 space-y-2">{cmdb.map(({ ci, relationship, impact }) => <div key={ci.id} className="rounded-lg border border-blue-100 bg-semantic-panel p-3 text-xs"><div className="font-bold text-slate-900">{ci.ciNumber} — {ci.name}</div><div className="mt-1 text-slate-600">{relationship.replaceAll('_', ' ')} · {ci.typeId} · {ci.environment} · {ci.criticality}</div><div className="mt-2 text-blue-800">{t('Affected business services:')} {impact.affectedBusinessServices?.length || 0}; {t('critical services:')} {impact.criticalServices?.length || 0}</div>{impact.affectedBusinessServices?.slice(0, 3).map((entry: any) => <div key={entry.ci.id} className="mt-1 font-mono text-[11px] text-slate-600">{t('Impact path:')} {entry.path?.map((id: string) => id).join(' → ')}</div>)}</div>)}</div>
        </div>
      )}
      ...
      {finding && (
        <div className="bg-slate-50/60 border border-slate-200 rounded-xl p-5 space-y-4 shadow-xs">
          <div className="flex items-center justify-between border-b border-slate-200 pb-3 flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
                {t('Security Finding Analysis & Telemetry')}
              </h3>
            </div>
            {finding.scannerSource && (
              <span className="px-2.5 py-0.5 rounded-full bg-semantic-panel text-slate-700 border border-slate-200 text-label font-mono font-medium shadow-xs">
                {t('Source:')} {finding.scannerSource}
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
            {finding.cweId && (
              <div className="p-3.5 bg-semantic-panel rounded-xl border border-slate-200 shadow-xs">
                <div className="text-slate-500 text-label font-medium">{t('CWE ID')}</div>
            ...
            {finding.cveId && (
              <div className="p-3.5 bg-semantic-panel rounded-xl border border-slate-200 shadow-xs">
                <div className="text-slate-500 text-label font-medium">{t('CVE Identifier')}</div>
            ...
            {finding.cvssScore && (
              <div className="p-3.5 bg-semantic-panel rounded-xl border border-slate-200 shadow-xs">
                <div className="text-slate-500 text-label font-medium">{t('CVSS v3.1 Score')}</div>
                <div className="font-mono font-bold text-amber-600 text-sm mt-0.5">{finding.cvssScore} / 10.0</div>
                <div className="text-label text-slate-400 mt-0.5">{t('Base Severity Metric')}</div>
              </div>
            )}
            {finding.owaspCategory && (
              <div className="p-3.5 bg-semantic-panel rounded-xl border border-slate-200 shadow-xs">
                <div className="text-slate-500 text-label font-medium">{t('OWASP Top 10')}</div>
                <div className="font-bold text-slate-800 text-sm truncate mt-0.5" title={finding.owaspCategory}>{finding.owaspCategory}</div>
                <div className="text-label text-slate-400 mt-0.5">{t('Application Security')}</div>
              </div>
            )}
          </div>
          ...
          {/* CVSS Vector */}
          {finding.cvssVector && (
            <div className="p-3 bg-semantic-panel rounded-lg border border-slate-200 text-xs font-mono flex items-center justify-between">
              <span className="text-slate-500 font-medium">{t('CVSS Vector:')}</span>
          ...
          {(finding.filePath || finding.endpoint) && (
            <div className="p-3.5 bg-semantic-panel rounded-lg border border-slate-200 space-y-2 text-xs">
              {finding.filePath && (
                <div className="flex items-center gap-2">
                  <FileCode className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                  <span className="text-slate-500 font-medium">{t('File:')}</span>
              ...
              {finding.endpoint && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-500 font-medium">{t('Endpoint:')}</span>
                  <span className="font-mono text-slate-800 font-semibold truncate">{finding.endpoint}</span>
                  {finding.httpParameter && <span className="text-slate-500 font-mono text-label">{t('Param:')} [{finding.httpParameter}]</span>}
                </div>
              )}
            </div>
          )}
          ...
          {finding.proofOfConcept && (
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold text-slate-600 uppercase tracking-wider">{t('Proof of Concept / Exploit Payload')}</div>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>{copiedPoc ? t('Copied') : t('Copy')}</span>
                </button>
              </div>
          ...
          {finding.remediationRecommendation && (
            <div className="p-4 bg-blue-50/70 border border-blue-200 rounded-lg space-y-1 text-xs">
              <div className="flex items-center gap-1.5 font-bold text-blue-800">
                ...
                <span>{t('Remediation Guidance')}</span>
              </div>
          ...
          {finding.compensatingControls && (
            <div className="p-4 bg-amber-50/70 border border-amber-200 rounded-lg space-y-1 text-xs">
              <div className="flex items-center gap-1.5 font-bold text-amber-800">
                ...
                <span>{t('Compensating Controls')}</span>
              </div>
        ...
        <div className="bg-slate-50/60 border border-slate-200 rounded-xl p-5 space-y-4 shadow-xs">
          ...
          {incident.mitreAttack && incident.mitreAttack.length > 0 && (
            <div className="space-y-2">
              <div className="text-xs font-bold text-slate-600 uppercase tracking-wider">{t('MITRE ATT&CK Techniques')}</div>
              <div className="flex flex-wrap gap-2">
                {incident.mitreAttack.map((m: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-semantic-panel border border-slate-200 text-xs font-mono shadow-xs">
          ...
          {/* IOCs */}
          <div className="p-4 bg-semantic-panel rounded-lg border border-slate-200 space-y-2 text-xs">
            <div className="font-bold text-slate-700 uppercase tracking-wider text-label">{t('Indicators of Compromise (IOCs)')}</div>
            {incident.iocs?.ipAddresses && incident.iocs.ipAddresses.length > 0 && (
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-slate-500 font-medium">{t('Suspicious IPs:')}</span>
            ...
            {incident.iocs?.urls && incident.iocs.urls.length > 0 && (
              <div className="flex items-center gap-2">
                <span className="text-slate-500 font-medium">{t('Target URLs:')}</span>
          ...
          {incident.containmentActions && (
            <div className="p-4 bg-semantic-panel border border-slate-200 rounded-lg space-y-1 text-xs">
              <div className="font-bold text-slate-800">{t('Containment Measures')}</div>
        ...
        <div className="bg-slate-50/60 border border-slate-200 rounded-xl p-5 space-y-4 shadow-xs">
          <div className="flex items-center justify-between border-b border-slate-200 pb-3 flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
                {t('Security Policy Exemption Details')}
              </h3>
            </div>
            <span className="px-2.5 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200 text-label font-mono font-bold">
              {t('Control:')} {exception.requestedControlId}
            </span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            <div className="p-4 bg-semantic-panel rounded-lg border border-slate-200">
              <div className="text-slate-500 font-bold uppercase tracking-wider text-label">{t('Business Justification')}</div>
              <div className="text-slate-800 mt-1.5 leading-relaxed">{exception.businessJustification}</div>
            </div>
            <div className="p-4 bg-semantic-panel rounded-lg border border-slate-200">
              <div className="text-slate-500 font-bold uppercase tracking-wider text-label">{t('Compensating Controls')}</div>
      ...
      {(application || asset) && (
        <div className="bg-slate-50/60 border border-slate-200 rounded-xl p-5 shadow-xs">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3 flex items-center gap-1.5">
            ...
            <span>{t('Affected Infrastructure')}</span>
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            {application && (
              <div className="p-4 bg-semantic-panel border border-slate-200 rounded-lg space-y-2 shadow-xs">
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 pt-1 text-label flex-wrap">
                  <span className="text-slate-500 font-medium">{t('Tech Stack:')}</span>
