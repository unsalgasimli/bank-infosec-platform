...
const LOADING_RE = /loading|yüklənir|Sessiyası Yoxlanılır|Loading secure workspace/i;
...
for (const [slug, route] of ROUTES) {
  ...
  for (let attempt = 1; attempt <= 4 && !captured; attempt++) {
    try {
      ...
      // wait for ANY loading indicator (page-level or inner) to clear, up to 45s
...
fs.writeFileSync(path.join(ROOT, 'crawl-report-inventory.json'), JSON.stringify(report, null, 1));
await browser.close();
