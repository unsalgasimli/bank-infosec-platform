...
// Only routes not yet validly captured in recapture-all (before the session died)
const ROUTES = [
  ...
  for (let attempt = 1; attempt <= 4 && !captured; attempt++) {
    try {
      ...
      if (LOGIN_RE.test(text)) {
        ...
        attempt--; // don't count a session restore as an attempt
        continue;
      }
...
fs.writeFileSync(path.join(ROOT, 'crawl-report-rest.json'), JSON.stringify(report, null, 1));
await browser.close();
